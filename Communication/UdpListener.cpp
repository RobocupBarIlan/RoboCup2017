...
void UdpListener::Listen() {
	...
	while (!m_stop) {
//		oldData.state = STATE_PLAYING;
//		newData.state = STATE_READY;
		...
		if (strcmp(m_header, "RGme") == 0)
			...
			type = unknown;
		//cout << "received " <<m_header<<endl;
		switch (type) {
		case RGme: {
			uint16_t version;
			memcpy(&version, m_byte_buffer + 4, sizeof(version));
			//cout<<"received: "<<version<<endl;
...
bool UdpListener::is_data_changed(RoboCupGameControlData * old_data, RoboCupGameControlData * new_data)
{
	...
	if (old_data->kickOffTeam != new_data->kickOffTeam) {
		...
		new_info.extraData = new_data->kickOffTeam;
	}
	...
	if (old_data->dropInTeam != new_data->dropInTeam) {
		...
		new_info.extraData = new_data->dropInTeam;
		return true;
	}
	...
	if (old_data->teams[0].teamNumber == TEAM_NUMBER) {
		if (old_data->teams[0].players[PLAYER_NUMBER].penalty != new_data->teams[0].players[PLAYER_NUMBER].penalty) {
			uint8_t secsTillUnpenalised = old_data->teams[0].players[PLAYER_NUMBER].secsTillUnpenalised;
			cout<<(int)new_data->teams[0].players[PLAYER_NUMBER].penalty<<endl;
			switch (new_data->teams[0].players[PLAYER_NUMBER].penalty)
			{
		...
		//if (old_data->teams[0].players[PLAYER_NUMBER].secsTillUnpenalised != new_data->teams[0].players[PLAYER_NUMBER].secsTillUnpenalised)	;
	}
	else {
		if (old_data->teams[1].players[PLAYER_NUMBER].penalty != new_data->teams[1].players[PLAYER_NUMBER].penalty) {
			uint8_t secsTillUnpenalised = old_data->teams[0].players[PLAYER_NUMBER].secsTillUnpenalised;
			switch (new_data->teams[1].players[PLAYER_NUMBER].penalty)
			{
			case HL_BALL_MANIPULATION:
				cout<<"HL_BALL_MANIPULATION"<<endl;
				new_info.messageNumber = RefereeInfo::Ball_Manipulation;
	...
	//if (old_data->teams[1].players[PLAYER_NUMBER].secsTillUnpenalised != new_data->teams[1].players[PLAYER_NUMBER].secsTillUnpenalised);
	return false;
}
