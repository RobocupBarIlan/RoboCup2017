...
public:
	...
	static vector<Vec4i> GetLines(Mat image, int threshold, int minLinLength, int maxLineGap); //The result will be set into the gc variable. please note that if no post was found all values in x,y of points will be set to -1. otherwise if only 1 post was found only the left post will be set to the appropriate value. (otherwise 2 posts-> update of both posts left and right).
	vector<vector<int>> GetJunctions(vector<Vec4i> linesP2, Mat frame, int dots_dis);
