 ...
 * static members init:
 */
BallNeuralNet BallDetector::BallDetectionNeuralNet(INPUT_SIZE_BALL_NEURAL_NET,  HIDDEN_LAYER_SIZE_BALL_NEURAL_NET,   OUTPUT_SIZE_BALL_NEURAL_NET,   PATH_INPUT_HIDDEN_WEIGHTS_BALL_NEURAL_NET,   PATH_HIDDEN_OUTPUT_WEIGHTS_BALL_NEURAL_NET);
...
//	try
//	{
	//Apply ball detection neural network to a given blob (in bw_mat):
	Mat upper_part_bw_mat(bw_mat, cv::Rect(0, 0, bw_mat.cols, floor((3*bw_mat.rows/4)))); //We want to get the upper part because the upper part of the ball looks like a better circle (or arc of circle) than the lower part which has shadowing...
	Size size(NEURAL_NET_INPUT_IMAGE_HEIGHT,NEURAL_NET_INPUT_IMAGE_WIDTH);//the dst image size,e.g.100x100
	Mat resized_upper_part_bw_mat;
	resized_upper_part_bw_mat=Mat::zeros(NEURAL_NET_INPUT_IMAGE_HEIGHT, NEURAL_NET_INPUT_IMAGE_WIDTH, CV_8UC1);
	resize(upper_part_bw_mat,resized_upper_part_bw_mat,resized_upper_part_bw_mat.size(),INTER_CUBIC);//resize image


	//Set resized_upper_part_bw_mat to 1's and 0's only matrix:
	for (int i=0;i<resized_upper_part_bw_mat.rows;i++) //Run over rows
	{
		for(int k=0;k<resized_upper_part_bw_mat.cols;k++) //Run over columns
		{
			if(resized_upper_part_bw_mat.at<uchar>(i,k)!=0)
			{
				resized_upper_part_bw_mat.at<uchar>(i,k)=255;
			}
		}
	}


		//imwrite( "resized_mat"+to_string(call_id)+".png", resized_upper_part_bw_mat );
		//imwrite( "original_mat"+to_string(call_id)+".png", bw_mat );
		call_id++;

	//Unroll the resized_upper_part_bw_mat matrix to a vector of length INPUT_SIZE_BALL_NEURAL_NET:
	double ball_neural_net_input_array[INPUT_SIZE_BALL_NEURAL_NET];//=new double[INPUT_SIZE_BALL_NEURAL_NET];
	for(int j=0;j<NEURAL_NET_INPUT_IMAGE_WIDTH;j++) //Run over columns
	{
		for (int i=0;i<NEURAL_NET_INPUT_IMAGE_HEIGHT;i++) //Run over rows
		{
				ball_neural_net_input_array[j*NEURAL_NET_INPUT_IMAGE_HEIGHT+i]=((resized_upper_part_bw_mat.at<uchar>(i,j)!=0)?1:0);
				//cout<<"resized_upper_part_bw_mat.at<int>(i,j):"<<ball_neural_net_input_array[i*NEURAL_NET_INPUT_IMAGE_WIDTH+j]<<endl;
		}
	}
	imshow("resized_upper_part_bw_mat",resized_upper_part_bw_mat);
	waitKey(1);
	correlation= BallDetector::BallDetectionNeuralNet.query(ball_neural_net_input_array,INPUT_SIZE_BALL_NEURAL_NET);
	//correlation=0;
	//cout << "correlation: " << correlation << "\n" << endl;
...
void BallDetector::CircleFitRansac(Mat& frame,Mat& frame_edges,vector<BallCandidateRansac>& found_circles,int min_radius,int max_radius)
{
	...
	const int THRESHOLD_DISTANCE=2; //up to 3 pixels of error between distance from checked circle's center and its center.
